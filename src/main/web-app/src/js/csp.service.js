...
angular.module("csp.service", [])
    .controller("CspService", CspService);

CspService.$inject = ['$scope',
    'CSPPlugin', '_'];

function CspService($scope, CSPPlugin, _) {

    var vm = this;
    vm.signContent = signContent;
    vm.getAllCerts = getAllCerts;
    vm.isCerts = isCerts;
    $scope.total = 1;
    $scope.certs = undefined;

    async function getAllCerts(){
        $scope.certs = await getCerts();
    }

    async function signContent(thumbprint) {
        var cert = $scope.certs.find(c=>c.certInfo.thumbprint === thumbprint)
        await signPack(cert);
    }

    function isCerts(){
        return !($scope.certs === undefined);
    }
    ...
    async function sign(cert) {
        ...
        try {
            for (let i = 0; i < $scope.total;) {
                i++
                let res =  await _sign(signParams);
                console.log("Подписано "+i+" из "+$scope.total)
            }
        } catch (e) {
